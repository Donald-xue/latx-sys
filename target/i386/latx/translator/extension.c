    ...
    if (ir1_opnd_has_base(opnd1)) {
        ...
        if (ir1_opnd_has_index(opnd1)) {
            if (ir1_opnd_scale(opnd1) != 1) {
                la_append_ir2_opnd2_em(LISA_MOV_ADDRX, base_opnd, base_opnd);
            } else {
        ...
        if (ir1_opnd_has_index(opnd1)) {
            ...
            la_append_ir2_opnd2_em(LISA_MOV_ADDRX, index_opnd, index_opnd);
        } else {
            /* no base and index */
        }
    }
...
void em_recognize_address(IR1_INST *pir1)
{
    ...
    // IR1_OPND *opnd1;
    // FOREACH_OPND_INIT(i_opnd);

    // FOREACH_SRC_OPND(i_opnd, pir1, opnd1) { emi_recognize_address(opnd1); }
    // FOREACH_DEST_OPND(i_opnd, pir1, opnd1) { emi_recognize_address(opnd1); }
